...
void scheduler::updateScheduleList()
{
    ...
    foreach (QString job, jobs)
    {
        ...
        if (jobData[2] == "never")
            m_ui.scanJobTableWidget->setItem(rowCount, 3, new QTableWidgetItem(tr("Never")));
        else {
            tempDateTime.setMSecsSinceEpoch(jobData[2].toLongLong());
            m_ui.scanJobTableWidget->setItem(rowCount, 3, new QTableWidgetItem(tempDateTime.toString(tr("dd.MM.yyyy 'at' hh:mm"))));
        }
        tempDateTime.setMSecsSinceEpoch(jobData[3].toLongLong());
        m_ui.scanJobTableWidget->setItem(rowCount, 4, new QTableWidgetItem(tempDateTime.toString(tr("dd.MM.yyyy 'at' hh:mm"))));
...
void scheduler::slot_profileSelectionChanged()
{
    ...
    m_ui.logFileLabel->setText(tr("Log-File : ") + logFile);
