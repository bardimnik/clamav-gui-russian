...
void clamav_gui::getVersion()
{
    ...
    QString systemInfo = "<div style='font-size:12px;line-height:20px;'><b>" + tr("Scanner:") + " <font color='navy'>" + scannerVersion +
                 "</font><br>" + tr("Database:") + " <font color='navy'>" + value + "</font><br>";
    systemInfo += tr("Date:") + " <font color='navy'>" + lastUpdate + "</font></b></div>";
    ...
    if (status == QProcess::CrashExit)
    {
        ...
        if (exitCode == 0)
        {
            ...
            if (exitCode == 1)
            {
                ...
                monochrome == true ? m_color = "#404040;color:white" : m_color = "#ff0000;color:white";
                m_scannerTab->setStatusBarMessage(tr("Scan-Process finished ...... an error occurred!"), m_color);
