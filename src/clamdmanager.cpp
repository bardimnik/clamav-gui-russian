...
void clamdManager::findClamdProcess()
{
    ...
    QString processStatus = runProg("ps", {"-o", "args=", "-p", clamdPid});
...
void clamdManager::slot_clamdStartStopButtonClicked()
{
    ...
    if (checkClamdRunning() == false)
    {
        ...
        m_ui.clamdIconLabel->setMovie(new QMovie(":/icons/icons/gifs/spinning_segments_small.gif", QByteArray(), this));
        ...
        parameters.clear();
        parameters << m_clamdLocation;
        parameters << "-c" << m_setupFile->getSectionValue("Clamd","ClamdConfPath");
        if (m_sudoGUI == "")
            m_sudoGUI = m_setupFile->getSectionValue("Settings", "SudoGUI");
        ...
        {
            QStringList accParams;
            accParams << m_clamonaccLocation;
            accParams << "-c" << m_setupFile->getSectionValue("Clamd","ClamdConfPath");
            accParams << "-l" << m_setupFile->getSectionValue("Clamd","ClamdLogFile");
            if (!clamonaccOptions.isEmpty())
                accParams << clamonaccOptions.split(" ", Qt::SkipEmptyParts);
            startProcess(m_restartClamonaccProcess, m_sudoGUI, accParams);
        }
        else {
        ...
        if (isValidPid(clamdPid)) {
            if (isRunningInFlatpak())
                QProcess::execute("flatpak-spawn", {"--host", "kill", "-TERM", clamdPid});
            else
                QProcess::execute(m_sudoGUI, {"kill", "-TERM", clamdPid});
        }

        QString clamonaccPid = m_setupFile->getSectionValue("Clamd","ClamonaccPid");
        if (clamonaccPid != "n/a" && isValidPid(clamonaccPid)) {
            if (isRunningInFlatpak())
                QProcess::execute("flatpak-spawn", {"--host", "kill", "-TERM", clamonaccPid});
            else
                QProcess::execute(m_sudoGUI, {"kill", "-TERM", clamonaccPid});
        }
    }
...
void clamdManager::slot_restartClamdButtonClicked()
{
    ...
    m_ui.clamdIconLabel_2->setMovie(new QMovie(":/icons/icons/gifs/spinning_segments_small.gif", QByteArray(), this));
    ...
    QString clamonaccPid = m_setupFile->getSectionValue("Clamd","ClamonaccPid");
    if ((clamonaccPid != "n/a") && (clamonaccPid != "") && isValidPid(clamonaccPid)) {
        if (isRunningInFlatpak())
            QProcess::execute("flatpak-spawn", {"--host", "kill", "-TERM", clamonaccPid});
        else
            QProcess::execute(m_sudoGUI, {"kill", "-TERM", clamonaccPid});
...
void clamdManager::restartClamonacc()
{
    ...
    if (isValidPid(m_clamonaccPid)) {
        if (isRunningInFlatpak())
            QProcess::execute("flatpak-spawn", {"--host", "kill", "-TERM", m_clamonaccPid});
        else
            QProcess::execute(m_sudoGUI, {"kill", "-TERM", m_clamonaccPid});
    }

    if (m_dirsUnderMonitoring > 0)
    {
        parameters.clear();
        parameters << m_clamonaccLocation;
        parameters << "-c" << m_setupFile->getSectionValue("Clamd","ClamdConfPath");
        parameters << "-l" << m_setupFile->getSectionValue("Clamd","ClamdLogFile");
        startProcess(m_startClamdProcess, m_sudoGUI, parameters);
    }
...
//--------------------------------------------------------
    ...
    for (int i = 0; i < clamdConfElement.length()-1; i++)
    {
        ...
        label.replace(QString(QChar(0x2010)) + " ", "");
