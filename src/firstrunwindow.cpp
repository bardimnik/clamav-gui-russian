...
{
    m_ui->setupUi(this);
    this->setWindowTitle(tr("ClamAV-GUI Basic Settings"));
    ...
    if (m_initIndex < m_initCommands.size())
    {
        switch (m_initIndex)
        {
            ...
            case 7:
                if (rc != "")
                {
                    m_ui->applicationUserLabel->setText(tr("Database Owner : ") + elements.at(0));
                    ...
                    m_setupFile->setSectionValue("RequiredApplications","User",m_ui->applicationUserLabel->text().replace(tr("Database Owner : "),""));
                }
                break;
            case 8:
                if (elements.count() > 0)
                {
                    m_ui->applicationGroupLabel->setText(tr("Application Group  : ") + elements.at(0));
                    m_ui->applicationGroupStatusLabel->setPixmap(QPixmap(":/icons/icons/create.png"));
                    m_setupFile->setSectionValue("RequiredApplications","Group",m_ui->applicationGroupLabel->text().replace(tr("Application Group  : "),""));
                }
                break;
            case 9:
